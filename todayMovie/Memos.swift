...
/// ------------------------------------------------------
/// `1월 22일 숙제`
/// 1. combine 제거하고 뷰모델이랑 다시 바인딩 하기 o
/// 2. 바인딩 한 뒤에, insert로 전체 업데이트 하지 않고 이쁘게 넣기 o
/// 3. pagination 안되는거 디버깅 후 처리 o
/// 3. 에러처리 다시 o
/// 4. image cache도 아예 다시 o
/// ------------------------------------------------------
...
///  
///  문제
///  1. Task, async/await, actor 다 제대로 모르고 사용했다.
///  2. 구조적인 문제
///     테스트가 불가능
///     싱글톤, 의존성 주입이 안된다.
///     테스트를 위해 로직을 바꾸면 안된다.
///     메모리 캐시에 캐시가 있으면 메모리에 있는 이미지를 반환하고, 리모트 이미지를 반환하지 않는다.
///     
